...
# scatterplot of average restaurant ratings for each city
us_rest %>%
  filter(!is.na(city))%>%
  group_by(city) %>%
    summarise(avg_rating=mean(rating))%>%
      arrange(avg_rating) %>%
        ...
        labs(y = "average rating", title = "Average Rating by City")
summary(us_rest$rating)

# boxplot of distance from downtown all restaruants are
us_rest %>%
  filter(!is.na(city))%>%
  group_by(city) %>%
        ggplot(aes(x=city, y=distance))+geom_boxplot()

# looking at the association between distance and rating for Chicago
us_rest %>%
  filter(!is.na(city), city == "Chicago")%>%
  group_by(city) %>%
        ggplot(aes(x=distance, y=rating)) + geom_point() # + geom_smooth()

# plot looks messy with all the of regression lines, but this shows the association
# between distance and rating without grouping by a city
us_rest %>%
  filter(!is.na(city))%>%
        ggplot(aes(x=distance, y=rating, color = price_2)) + geom_point(position = "jitter") + labs(title = "Assocation between Distance and Rating") #+ geom_smooth() 
cor(us_rest$distance, us_rest$rating)

# relationship between review count and rating relative to price of restaurant
us_rest %>%
  filter(!is.na(city))%>%
...
# Score/error
MLmetrics::Accuracy(y_true = test$rating, y_pred = y_hat) # getting error because there are 5 levels in the the training set and only 4 levels in the test set
